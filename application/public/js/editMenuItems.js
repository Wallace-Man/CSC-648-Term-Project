...
window.addEventListener("DOMContentLoaded", async (event) => {
    ...
    form.addEventListener("submit", async (event) => {
      ...
      items.forEach(item => {
        ...
        } else if (itemId === "") { // Assuming empty id field means a new item
        ...
        try {
          const response = await fetch('/addMenuItem', {
            ...
            body: JSON.stringify({
              ...
              price: item.price,
              category: item.category // if the category field exists
            })
          });
      ...
      for (const item of updatedItems) {
        try {
          const response = await fetch(`/editMenuItem/${item.itemId}`, { // This matches your edit route in menu.js
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
      ...
      for (const item of deletedItems) {
        try {
          const response = await fetch(`/deleteMenuItem/${item.itemId}`, { // This matches your delete route in menu.js
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            }
      ...
      window.location.href = '/restaurantInfo'; // redirect to the menu page after operations are completed
    });
